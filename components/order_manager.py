...
NON_RETRYABLE_ERROR_CODES = [
    ...
    110044, # SL/TP order type invalid
    110045, # SL/TP limit price invalid
]
...
class OrderManager:
    # --- [CORREÇÃO] Aceitar 'config' para obter parâmetros de slippage ---
    def __init__(self, symbol: str, testnet: bool = True, config: dict = None):
        self.symbol = symbol
    ...
    # --- API Calls V5 ---
    @retry(**RETRY_POLICY)
    # --- [CORREÇÃO] Aceitar sl_limit_price ---
    def _send_new_order_to_api(self, order: Order, sl_limit_price: float = None):
        """Usa place_order da V5, com suporte a Stop-Limit."""
        ...
        # --- [NOVO] Lógica do Stop-Limit ---
        sl_order_type = "Market"
        sl_limit_price_str = None
        if sl_str and sl_limit_price and sl_limit_price > 0:
            sl_order_type = "Limit"
            sl_limit_price_str = str(sl_limit_price)
            logger.debug(f"Usando Stop-Limit: Trigger={sl_str}, Limit={sl_limit_price_str}")
        # ------------------------------------
        ...
        return self.rest_session.place_order(
            ...
            stopLoss=sl_str, 
            takeProfit=tp_str,
            slOrderType=sl_order_type, # <-- NOVO
            slLimitPrice=sl_limit_price_str # <-- NOVO
            # positionIdx=0
        )

    # --- [CORREÇÃO] Aceitar sl_limit_price ---
    def _execute_new_order(self, order: Order, sl_limit_price: float = None):
        ...
        with SessionLocal() as db_session:
            try:
                ...
                # --- [CORREÇÃO] Passar sl_limit_price para a API ---
                response = self._send_new_order_to_api(order, sl_limit_price)
    ...
    def _send_close_order_to_api(self, closing_order: Order):
        # (Sem alterações - Fechamento de mercado não usa stop-limit)
    ...
    @retry(**RETRY_POLICY)
    # --- [CORREÇÃO] Aceitar new_sl_limit_price ---
    def _send_modify_order_to_api(self, mod_request: Order, new_sl_limit_price: float = None):
        """Usa set_trading_stop da V5, com suporte a Stop-Limit."""
        ...
        sl_order_type = "Market"
        sl_limit_price_str = None
        if sl_str != "0" and new_sl_limit_price and new_sl_limit_price > 0:
            sl_order_type = "Limit"
            sl_limit_price_str = str(new_sl_limit_price)
            logger.debug(f"Modificando para Stop-Limit: Trigger={sl_str}, Limit={sl_limit_price_str}")
        # ------------------------------------
    ...
    # --- [CORREÇÃO] Aceitar new_sl_limit_price ---
    def _execute_modify_order(self, mod_request: Order, new_sl_limit_price: float = None):
        ...
        try:
            # --- [CORREÇÃO] Passar new_sl_limit_price ---
            response = self._send_modify_order_to_api(mod_request, new_sl_limit_price)
    ...
    def _on_new_order_request(self, message):
        ...
        try:
            ...
            # --- [CORREÇÃO] Ler sl_limit_price ---
            sl_limit_price = data.get('sl_limit_price')
            ...
            if not all([order_req.symbol, order_req.side, order_req.qty is not None and order_req.qty > 0]):
                ...
                # --- [CORREÇÃO] Passar sl_limit_price ---
                self._execute_new_order(order_req, sl_limit_price)
    ...
    def _on_modify_order_request(self, message):
        ...
        try:
            ...
            # --- [CORREÇÃO] Ler new_sl_limit_price ---
            new_sl_limit_price = data.get('new_sl_limit_price')
            ...
            if not mod_req.symbol:
                ...
                # --- [CORREÇÃO] Passar new_sl_limit_price ---
                self._execute_modify_order(mod_req, new_sl_limit_price)
