...
class Order(Base):
    ...
    client_order_id = Column(String, unique=True, index=True) # ID gerado por nós (GARANTIA DE UNICIDADE)
    
    # --- MELHORIA DE ROBUSTEZ ---
    # O order_id da exchange pode não ser único (ex: ordens rejeitadas)
    # ou ser nulo. A unicidade é garantida pelo client_order_id.
    order_id = Column(String, unique=False, index=True, nullable=True) # ID retornado pela Bybit
    # -----------------------------
